"""YITING Safety Reviewer — local incident-room runtime + Qwen.

Independent cross-checker. ALWAYS participates in every incident.
Issues Verdict: CONFIRM, CHALLENGE, FALSE_ALARM, or NEEDS_HUMAN.

All advisory LLM calls route through Qwen Cloud. Deterministic cross-checks
...
Architecture:
  ...
  - LocalRoomAgent invokes the Qwen-assisted review callback after acceptance
...
Honest claims:
  - "Qwen-backed": Qwen improves verdict wording when credentials are set
...
logger = logging.getLogger("yiting.safety_reviewer")

# Gateway URL for SubmissionClient
GATEWAY_URL = os.getenv("GATEWAY_URL", "http://localhost:8000")

# Evidence sources that Diagnosis should have queried
REQUIRED_EVIDENCE_SOURCES = frozenset({"sentry", "metrics", "deploys", "uptime"})

# Minimum evidence_strength thresholds per severity
# If Diagnosis claims P1 but evidence_strength is below threshold → CHALLENGE
...
@dataclass
class ReviewContext:
    """Trusted context for an incident under review.
...
# Rooms where Safety Reviewer has already published its terminal Verdict.
...
def cross_check_assessment(assessment: Assessment) -> dict[str, Any]:
    ...
    Returns a dict with:
      ...
      - has_temporal_correlation: bool (deploy-to-error gap present)
    ...
    # 5. Temporal correlation (deploy-to-error gap)
...
class SubmitVerdict(BaseModel):
    """Submit the Safety Reviewer's verdict for an incident.

    MUST be called after analyzing the Assessment card. The verdict will be
    submitted to Gateway and published to incident room, with decision-specific routing:
    - CONFIRM: recruit Commander, @mention Commander with Verdict
    - CHALLENGE: @mention Diagnosis with specific evidence request
    ...
    decision: str = Field(
        description=(
            ...
            "CHALLENGE (evidence gaps are actionable, send back to Diagnosis), "
            "FALSE_ALARM (incident is clearly noise), or "
    ...
    agrees_with_diagnosis: bool = Field(
        description="Whether this verdict agrees with the Diagnosis assessment"
    )
    challenge_request: str = Field(
        default="",
        description=(
            "When decision is CHALLENGE: specific evidence or investigation "
            "request for Diagnosis to address. Required for CHALLENGE."
        )
    )
    suppression_advice: str = Field(
        ...
        description=(
            ...
            "pattern for future similar alerts."
        )
    )
...
async def handle_submit_verdict(input: SubmitVerdict) -> str:
    ...
    5. Decision-specific routing:
       - CONFIRM: recruit Commander → @mention Commander with Verdict
       - CHALLENGE: @mention Diagnosis (already in room) with challenge request
    ...
    if ctx is None:
        return f"Error: unknown incident {input.incident_id}. Cannot submit verdict."
    ...
    if ctx.force_needs_human and decision != "NEEDS_HUMAN":
        logger.info(
            ...
            f"for {input.incident_id} (max challenges exhausted)"
        )
        decision = "NEEDS_HUMAN"

    async with ctx.lock:
        if ctx.submitted:
            return f"Verdict already submitted for {input.incident_id}."
        ...
        if cross_check["issues"] and decision == "CONFIRM":
            original_decision = decision
            decision = "CHALLENGE"
            logger.warning(
                f"[safety_reviewer] DETERMINISTIC OVERRIDE: {original_decision} → "
                f"CHALLENGE for {input.incident_id}. "
        ...
        try:
            ...
            async with SubmissionClient(
                ...
                # Diagnosis flow: prepare → recruit Reviewer → publish → confirm.
                # Reviewer can receive the Assessment BEFORE Diagnosis calls confirm(),
                    ...
                    try:
                        ...
                        if e.status_code == 409 and attempt < len(STATE_RETRY_DELAYS) - 1:
                            logger.warning(
                                ...
                                f"Waiting {delay}s for Diagnosis to confirm ASSESSED..."
                            )
                ...
                try:
                    ...
                    if decision == "CONFIRM":
                        # --- CONFIRM: recruit Commander, @mention Commander ---
                        ...
                        logger.info(
                            f"[safety_reviewer] Recruited Commander "
                            f"{commander_id[:12]}... into room "
                            f"{publish_room[:12]}..."
                        )

                        # Publish Verdict @mentioning Commander
                        ...
                        # --- CHALLENGE: @mention Diagnosis (already in room) ---
                        ...
                        suppression_text = (
                            f"{sealed_message}\n\n"
                            f"🔇 **FALSE ALARM** — Incident suppressed.\n"
                        )
                        ...
                        alert_fp = getattr(input, "fingerprint", "") or ""
                        if alert_fp:
                                ...
                                async with httpx.AsyncClient(timeout=5) as gw_client:
                                    rule_resp = await gw_client.post(
                                        ...
                                        json={
                                            ...
                                            "source_incident_id": input.incident_id,
                                        },
                        ...
                        escalation_text = (
                            ...
                            f"This incident requires human judgment. "
                ...
                logger.info(
                    ...
                    f"incident={input.incident_id}, decision={decision}, "
            ...
            if decision != "CHALLENGE":
                ...
                logger.info(
                    ...
                    f"for incident {input.incident_id} — context remains open "
                    f"for revised Assessment"
                )

            return (
                f"Verdict submitted successfully for {input.incident_id}. "
                f"Decision: {decision}. "
...
async def run_local_safety_review(event) -> None:
    """Run Safety Reviewer without an external adapter after acceptance."""
    ...
    qwen = await ask_qwen_json(
        ...
        system=(
            "You are Zhou Shen, the Safety Reviewer. Explain the deterministic "
    ...
    if qwen:
        reasoning = bounded_text(qwen.get("reasoning"), max_len=800) or reasoning
        if decision == "CHALLENGE":
            challenge_request = (
                bounded_text(qwen.get("challenge_request"), max_len=600)
...
async def _submit_deterministic_verdict(ctx: ReviewContext) -> None:
    ...
    normal CONFIRM path, but bypasses the LLM entirely. Posts to the incident
    ...
    async with SubmissionClient(
        ...
        # 2. Publish to incident room with human approver mentions
        room_client = IncidentRoomClient(
            sender_id=os.getenv("SAFETY_REVIEWER_AGENT_ID", "safety_reviewer"),
            sender_role="safety_reviewer",
        )
        ...
        logger.info(
            ...
            f"incident={ctx.incident_id}, state={confirm.new_state}"
        )
    ...
    logger.info(
        f"[safety_reviewer] Marked room {ctx.room_id[:12]}... for "
        f"post-handoff silence (deterministic verdict, incident {ctx.incident_id})"
    )


# ---------------------------------------------------------------------------
# Safety Reviewer Preprocessor (thin: validate → store context → delegate)
# ---------------------------------------------------------------------------

class SafetyReviewerPreprocessor:
    """Thin preprocessor for the Safety Reviewer agent.

    Intercepts Assessment messages from incident room. Validates sender (must be
    Diagnosis), checks seal fields, Pydantic-validates, stores trusted
    per-incident context, and delegates to the local runtime for
    tool orchestration.
    ...
    async def process(self, ctx, event, **kwargs):
        ...
        if sender_id != self._diagnosis_agent_id:
            logger.warning(
                ...
                f"{sender_id!r} — expected Diagnosis {self._diagnosis_agent_id!r}"
            )
            return None
        ...
        if ACTIVE_INCIDENTS and incident_id_guard not in ACTIVE_INCIDENTS:
            logger.info(f"[safety_reviewer] Skipping non-active incident {incident_id_guard}")
            return None

        # ----- Stale Card Guard (cost optimization) -----
        card_seq = card_data.get("sequence_number")
        if incident_id_guard and await should_skip_stale_card(incident_id_guard, card_seq, "safety_reviewer"):
            return None
        if incident_id_guard and await should_skip_terminal_incident(incident_id_guard, role="safety_reviewer"):
            return None
        ...
        confirmed_cards = await fetch_confirmed_cards(
            incident_id, agent_key=submission_key, role="safety_reviewer",
        )
        if confirmed_cards and assessment_already_reviewed(confirmed_cards, card_data):
            logger.info(
                "[safety_reviewer] Skipping stale Assessment for %s: later "
                "Verdict already reviewed it",
                incident_id,
            )
            return None
        ...
        if existing is not None:
            if existing.submitted:
                logger.warning(
                    ...
                    f"incident {incident_id} — ignoring (prevents duplicate Verdict)"
                )
                return None
            ...
            if assessment_revision > existing.revision:
                if existing.challenge_count >= 2:
                    logger.warning(
                        ...
                        f"incident {incident_id} — deterministically submitting "
                ...
                logger.info(
                    ...
                    f"for incident {incident_id} (previous: v{existing.revision}, "
                ...
                logger.warning(
                    ...
                    f"for incident {incident_id} (current: v{existing.revision}) — ignoring"
                )
                return None
        else:
            # After a process restart the in-memory challenge budget is gone;
            # derive it from the sealed CHALLENGE Verdicts in the room ledger
            # so a restart can never grant extra challenges.
            prev_challenge_count = self._restore_challenge_count_from_cards(confirmed_cards)
            prev_force_needs_human = prev_challenge_count >= 2
            if prev_force_needs_human and assessment_revision > 1:
                # Mirror the in-memory max-challenge branch: deterministic
                # NEEDS_HUMAN without spending an LLM review.
                restored = ReviewContext(
                    incident_id=incident_id,
                    room_id=room_id,
                ...
                logger.warning(
                    f"[safety_reviewer] Max challenges (2) reached for "
                    f"incident {incident_id} (restored from ledger) — "
                    f"deterministically submitting NEEDS_HUMAN verdict"
                )
                try:
                    await _submit_deterministic_verdict(restored)
                except Exception as exc:
                    logger.error(
                        "[safety_reviewer] Deterministic NEEDS_HUMAN failed "
                        "for %s (%s)",
                ...
                return None
            if prev_challenge_count:
                logger.info(
                    f"[safety_reviewer] Restored challenge budget for "
                    f"{incident_id} from Gateway ledger "
                    f"(sealed_challenges={prev_challenge_count})"
                )

        # ----- Store trusted context -----
        _trusted_context[incident_id] = ReviewContext(
            incident_id=incident_id,
            room_id=room_id,
            room_message_id=room_message_id,
            source_card_hash=card_data.get("card_hash", ""),
        ...
        logger.info(
            ...
            f"{incident_id} (challenge_count={prev_challenge_count}). "
            f"Context stored. Delegating to local Qwen review."
        )

        # ----- Delegate to local incident-room runtime -----
        return await self._default_preprocessor.process(ctx, event, **kwargs)

    async def _restore_challenge_count(self, incident_id: str) -> int:
        """Count sealed CHALLENGE Verdicts at the Gateway.

        The room ledger is the durable challenge budget; in-memory state is
        only a cache of it. Fail-open to 0 (fresh incident / Gateway blip):
        the Gateway's state machine remains the authoritative backstop.
        """
        submission_key = os.getenv(
            "SAFETY_REVIEWER_SUBMISSION_KEY",
            os.getenv("GATEWAY_SECRET", ""),
        ...
        return self._restore_challenge_count_from_cards(cards)

    @staticmethod
    def _restore_challenge_count_from_cards(cards: list[dict]) -> int:
        """Count sealed CHALLENGE cards from a fetched ledger snapshot."""
        return count_challenge_verdicts(cards)


# ---------------------------------------------------------------------------
# System prompt retained for Qwen reviewer behavior
# ---------------------------------------------------------------------------

SAFETY_REVIEWER_SYSTEM_PROMPT = """\
You are the Safety Reviewer in the YITING incident command system.
Your role is to independently verify every diagnosis before it reaches remediation.

When you receive an Assessment card from Diagnosis in a room message, you MUST:

1. **Parse the Assessment** from the message:
   - Extract incident_id, severity, evidence_strength, root_cause_hypothesis
   - Note the evidence sources and their anomaly/relevance data

2. **Cross-check the evidence independently**:
   a) Were ALL 4 evidence sources queried? (sentry, metrics, deploys, uptime)
   ...
   e) Is there temporal correlation between deploys and errors?
...
evidence strength. Set agrees_with_diagnosis=true.
   - **CHALLENGE**: Evidence gaps exist AND are actionable (Diagnosis can fix them). \
Provide specific challenge_request describing what Diagnosis should re-investigate. \
PREFER this over CONFIRM when evidence is weak. Set agrees_with_diagnosis=false.
   - **FALSE_ALARM**: The incident is clearly noise — no anomalies detected, very low \
...
Set agrees_with_diagnosis based on whether you agree with Diagnosis's analysis.

4. **Call submitverdict** with:
   - The exact incident_id from the Assessment
...
- FALSE_ALARM when the incident is clearly noise
- NEEDS_HUMAN only for situations no agent can resolve

You do NOT trust Diagnosis — you verify independently.
...
async def create_safety_reviewer_agent():
    """Create the Safety Reviewer agent on the Gateway-owned room runtime."""
    ...
    if missing_critical:
        raise RuntimeError(
            f"Safety Reviewer agent cannot start: missing required env vars: "
