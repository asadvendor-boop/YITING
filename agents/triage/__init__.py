"""YITING Triage Agent — local incident-room runtime + Qwen.

Classifies incoming alerts, filters noise, applies deterministic guards,
and routes genuine incidents to Diagnosis via a sealed TriageDecision.

Architecture:
  - Preprocessor intercepts AlertCard messages from the incident room runtime
  - Validates sender identity (must be Recorder from trusted agent registry)
  - Validates card structure via Pydantic (AlertCard model)
  - Calls Qwen for noise classification
  ...
  - Only recruits Diagnosis when decision="route" (not on suppress)
  - Non-AlertCard messages pass through to the default LLM adapter

Security:
  - Sender validation: only accepts AlertCards from the registered Recorder UUID
...
logger = logging.getLogger("yiting.triage")

# Gateway URL for SubmissionClient (same pattern as tracer_bullet.py)
GATEWAY_URL = os.getenv("GATEWAY_URL", "http://localhost:8000")

# ----- Constants -----
TRIAGE_SYSTEM_PROMPT = """You are YITING Triage, an incident classification agent.

Your job: given an AlertCard from a monitoring system, determine whether this
alert represents a genuine incident that needs investigation, or noise that
...
Guidelines:
- noise_score 0.0 = definitely a real incident
- noise_score 1.0 = definitely noise
- If noise_score < 0.5, decision should be "route"
- If noise_score >= 0.7, decision can be "suppress"
- P1 severity or security-relevant alerts should ALWAYS be "route" with low noise_score
...
def _validate_alert_card(card_data: dict) -> AlertCard | None:
    """Validate card_data against the AlertCard Pydantic model.

    Returns a validated AlertCard instance, or None if validation fails.
    """
    try:
        return AlertCard(**card_data)
    except Exception as exc:
        logger.warning(
            "[triage] AlertCard validation failed (%s)",
            type(exc).__name__,
        )
        return None
...
def _apply_deterministic_guards(
    ...
    severity = alert_card.get("preliminary_severity", "unknown")
    security = alert_card.get("security_relevant", False)
...
# Rooms where Triage has already submitted its sealed TriageDecision.
# Non-card Agent messages in these rooms are silently consumed.
_handoff_rooms: set[str] = set()


class TriagePreprocessor:
    """Deterministic preprocessor for the Triage agent.

    Intercepts room messages containing AlertCards. When an AlertCard is
    ...
    6. Only on decision="route": recruits Diagnosis into the room
    7. Returns None (consuming the message — no further LLM processing)

    Non-AlertCard messages from agents are silently consumed after
    handoff (post-handoff silence). Non-AlertCard messages from
    ...
    def __init__(self, llm, triage_agent_id: str, triage_api_key: str):
        ...
        # Trusted sender: only Recorder may send AlertCards
        self._recorder_agent_id = get_agent_ids().get("recorder", "")

        self._room_client = IncidentRoomClient(
    ...
    async def _post_to_room(
        ...
        """Post a message to a Gateway-owned incident room.

        Returns the incident-room message id.
        """
    ...
    async def _add_participant(self, room_id: str, agent_id: str) -> None:
        """Add an agent to an incident room (for dynamic recruitment).
    ...
    async def _classify_with_llm(self, alert_card: dict) -> dict:
        """Call Qwen to classify the alert.
        ...
        alert_summary = (
            f"AlertCard:\n"
            f"  Title: {alert_card.get('title', 'unknown')}\n"
            f"  Source: {alert_card.get('source', 'unknown')}\n"
            f"  Severity: {alert_card.get('preliminary_severity', 'unknown')}\n"
            f"  Security Relevant: {alert_card.get('security_relevant', False)}\n"
            f"  Alert ID: {alert_card.get('alert_id', 'unknown')}\n"
        )

        # Add raw_payload summary if present
        raw = alert_card.get("raw_payload", {})
        if raw:
            # Truncate raw payload to avoid token overflow
            raw_str = json.dumps(raw, default=str)[:500]
            alert_summary += f"  Raw Payload (truncated): {raw_str}\n"
        ...
        try:
            ...
            messages = [
                ...
                HumanMessage(content=alert_summary),
            ]
    ...
    async def _submit_triage_decision(
        ...
        # Get incident_id — AlertCard uses alert_id as the incident key via Gateway
        incident_id = alert_card.get("incident_id") or alert_card.get("alert_id", "")
        alert_id = alert_card.get("alert_id", "")
        ...
        source_card_hash = alert_card.get("card_hash", "")
        ...
        async with SubmissionClient(
            ...
            logger.info(
                ...
                f"incident={prepared.incident_id}"
            )

            # Step 2: Publish sealed card to the incident room
            ...
            if decision == "route":
                if not self._diagnosis_agent_id:
                    raise RuntimeError(
                        ...
                        "Route decision requires a Diagnosis agent to hand off to."
                    )
                ...
                # Suppress: mention Recorder for incident-room delivery
            ...
            if not mentions:
                raise RuntimeError(
                    "[triage] No mentions resolved for incident-room publication."
                )
            ...
            logger.info(
                f"[triage] Published sealed TriageDecision to incident room: "
    ...
    async def process(self, ctx, event, **kwargs):
        ...
        # Only handle MessageEvents (same pattern as Operator preprocessor)
        ...
        # Try to extract an AlertCard from the message
        ...
        if card_data.get("card_type") != "AlertCard":
        ...
        # ----- Sender Validation -----
        # Only the registered Recorder agent may send AlertCards.
        # Forged AlertCards from other participants are rejected.
        if sender_type != "Agent":
            logger.warning(
                f"[triage] REJECTED AlertCard from non-agent "
                f"sender_type={sender_type!r}, sender_id={sender_id!r}"
            )
            return None

        if not self._recorder_agent_id:
            # Fail-closed: if Recorder ID is not configured, reject ALL AlertCards.
            # Never trust an AlertCard when we can't verify the sender.
            logger.error(
                "[triage] REJECTED AlertCard: RECORDER_AGENT_ID not configured. "
        ...
        if sender_id != self._recorder_agent_id:
            logger.warning(
                f"[triage] REJECTED AlertCard from untrusted agent "
        ...
        incident_id = card_data.get("alert_id", "")
        if ACTIVE_INCIDENTS and incident_id not in ACTIVE_INCIDENTS:
            logger.info(f"[triage] Skipping non-active incident {incident_id}")
            return None

        # ----- Stale Card Guard (cost optimization) -----
        # Skip if a higher-seq card is already published for this incident.
        card_seq = card_data.get("sequence_number")
        if incident_id and await should_skip_stale_card(incident_id, card_seq, "triage"):
            return None
        if incident_id and await should_skip_terminal_incident(incident_id, role="triage"):
            return None
        if incident_id:
            cards = await fetch_confirmed_cards(
                incident_id,
                agent_key=self._submission_key or os.getenv("GATEWAY_SECRET", ""),
                role="triage",
            )
            if cards and alert_already_triaged(cards, card_data):
                logger.info(
                    "[triage] Skipping stale AlertCard for %s: later "
                    "TriageDecision already exists",
                    incident_id,
                )
                return None

        # ----- Pydantic Validation -----
        validated_card = _validate_alert_card(card_data)
        if validated_card is None:
            logger.warning(
                f"[triage] REJECTED malformed AlertCard from {sender_id[:12]}..."
            )
            return None

        # ----- AlertCard Processing Pipeline -----
        logger.info(
            f"[triage] Received AlertCard from Recorder/{sender_id[:12]}...: "
            f"alert_id={card_data.get('alert_id', '?')}, "
        ...
        try:
            ...
            logger.info(
                ...
                f"  Alert: {card_data.get('alert_id', '?')}\n"
                f"  Decision: {decision}\n"
            ...
            logger.error(
                "[triage] Failed to process AlertCard (%s)",
                type(exc).__name__,
            )
            # Don't crash the agent — swallow and log
            # The alert will remain at DETECTED, and can be retried
        ...
        if room_id:
            ...
            logger.info(
                ...
                f"post-handoff silence (incident {card_data.get('alert_id', '?')})"
            )

        # Consume the message — don't pass AlertCards to the LLM adapter
        return None


async def create_triage_agent():
    """Create the Triage agent on the Gateway-owned incident-room runtime."""
    from langchain_openai import ChatOpenAI

    config = MODELS["triage"]

    provider = get_provider_settings()["qwen"]

    # LLM: Qwen via Alibaba Cloud Model Studio's DashScope-compatible endpoint.
    llm = ChatOpenAI(
        model=config.model,
    ...
    # Preprocessor: intercepts AlertCards before the LLM
    ...
    # (catches before demo, not mid-incident)
    required_vars = {
