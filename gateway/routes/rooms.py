"""Gateway-owned incident room API.

These routes are the local collaboration substrate for the Alibaba/Qwen
deployment. They store rooms, participants, and messages in the Gateway ledger.
"""
...
def store_room_participant(
    ...
    *,
    ...
    """Insert or update an incident-room participant."""
    room = db.execute("SELECT room_id FROM incident_rooms WHERE room_id=?", (room_id,)).fetchone()
    if not room:
...
def store_room_message(
    ...
    *,
    ...
    """Append a message to a Gateway-owned incident room."""
    room = db.execute("SELECT * FROM incident_rooms WHERE room_id=?", (room_id,)).fetchone()
    if not room:
        raise HTTPException(status_code=404, detail="room_not_found")
    ...
    row = db.execute(
        "SELECT * FROM incident_room_messages WHERE message_id=?",
        (message_id,),
    ).fetchone()
...
async def create_room(
    ...
    if body.incident_id:
        existing = db.execute(
            "SELECT * FROM incident_rooms WHERE incident_id=?",
            (body.incident_id,),
        ).fetchone()
    if existing:
    ...
    try:
        ...
        db.execute(
            ...
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (room_id, body.incident_id, body.title, role, now, now),
        )
        if body.incident_id:
            db.execute(
                """
                UPDATE incidents
                SET room_id=?, room_alias_id=COALESCE(room_alias_id, ?), updated_at=?
                WHERE incident_id=?
                """,
                (room_id, room_id, now, body.incident_id),
            )
        db.commit()
    except Exception:
        db.rollback()
    ...
    row = db.execute("SELECT * FROM incident_rooms WHERE room_id=?", (room_id,)).fetchone()
...
async def list_rooms(
    ...
    """List incident rooms visible to an agent.
    ...
    if participant_id:
        join = """
        JOIN incident_room_participants p
    ...
    rows = db.execute(
        ...
        LEFT JOIN incident_room_messages m ON m.room_id = r.room_id
        {join}
        {where}
        GROUP BY r.room_id
    ...
    for row in rows:
        room = _room_to_response(row)
        room["incident_state"] = row["incident_state"]
