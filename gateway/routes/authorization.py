"""YITING Gateway — Authorization Routes (Fork B: Gateway-owned).

POST /api/authorization/request — Commander requests policy authorization for low-risk plans.
...
logger = logging.getLogger("yiting.authorization")
...
class PolicyAuthRequest(BaseModel):
    """Commander requests policy authorization for a low-risk plan."""
    incident_id: str
    plan_hash: str
...
class AuthorizationConsumeRequest(BaseModel):
    incident_id: str  # ONLY incident_id - Gateway derives everything
...
async def request_policy_authorization(
    ...
    the request body. The Commander only provides incident_id + plan_hash.

    Prerequisites:
        - Incident must be in PLANNED state (ResponsePlan confirmed)
        - Stored ResponsePlan risk_level must be "low" or "medium"
        - requires_human_approval must be False
        - Commander role verified via X-Agent-Key
    ...
    idem_key = derive_idempotency_key(
        "gateway_policy_auth", body.incident_id, body.plan_hash,
    )

    existing_card = db.execute(
        "SELECT card_json, card_hash FROM cards "
        "WHERE idempotency_key=? AND incident_id=? AND card_type='PolicyAuthorization'",
        (idem_key, body.incident_id),
    ).fetchone()
    ...
    if existing_card:
        ...
        if status == "PUBLISHED":
            inc_row = db.execute("SELECT state FROM incidents WHERE incident_id=?", (body.incident_id,)).fetchone()
    ...
    if incident["state"] != "PLANNED":
        raise HTTPException(
            status_code=409,
            detail=f"Incident state is {incident['state']!r}, expected 'PLANNED'. "
    ...
    if not plan_card:
        raise HTTPException(
            status_code=409,
            detail="No confirmed ResponsePlan found for this incident.",
        )
    ...
    # The Commander computes plan_hash from model_dump() BEFORE seal_card
    # adds chain fields (card_hash, previous_card_hash, sequence_number).
    # The stored card_json has seal-set values for those fields.
    # To match the Commander's hash, reset chain fields to pre-seal defaults.
    ...
    stored_risk = plan_data.get("risk_level", "high")
    ...
    if needs_seal:
        ...
        try:
            ...
            db.execute(
                ...
                (authorization_id, body.incident_id, body.plan_hash,
    ...
    # --- Post-commit: post sealed card to the Gateway-owned incident room ---
    ...
    # Fetch room_id from the incident record. ``room_alias_id`` is retained as
    # a compatibility alias during schema cleanup.
    inc_row = db.execute(
    ...
    if not room_id:
        logger.error(
            f"[authorization] No room_id for incident {body.incident_id} — "
            f"cannot publish PolicyAuthorization"
        )
        raise HTTPException(
            status_code=502,
            detail="Cannot publish PolicyAuthorization: no incident room for this incident. "
            "Authorization is PENDING — retry is safe.",
        )

    # Use Recorder identity (system/gateway role), NOT Commander's identity.
    ...
    row = db.execute(
        ...
        (sealed_card_hash, body.incident_id),
    ).fetchone()
    ...
    try:
        ...
        logger.error(
            "[authorization] Incident-room publication failed for incident %s (%s); "
        ...
        raise HTTPException(
            status_code=502,
            detail="PolicyAuthorization was sealed but incident-room publication failed. "
    ...
    advance_authorization_lifecycle(
        ...
        target_incident_state="AUTHORIZED"
    )

    logger.info(
        f"[authorization] PolicyAuthorization issued + published to incident room: "
        f"incident={body.incident_id}, auth_id={authorization_id[:12]}..., "
...
# ---------------------------------------------------------------------------
# Consume authorization (Operator consumes a PUBLISHED authorization)
# ---------------------------------------------------------------------------
...
async def consume_authorization(
    ...
    """Operator consumes a PUBLISHED PolicyAuthorization.
    ...
    # Crash-recovery replay: a restarted Operator re-consumes after the
    # PUBLISHED->CONSUMED flip committed but before execution finished.
    # Re-issuing the identical payload to the SAME consumer role grants no new
    # authority — hash binding and expiry are still enforced below, and the
    # victim app's already_applied guard keeps re-execution idempotent.
    # A different consumer replaying is still refused.
    is_replay = auth_status == "CONSUMED" and (auth_row["consumed_by"] or "") == role_or_error
    if auth_status == "CONSUMED" and not is_replay:
        raise HTTPException(status_code=409, detail="already_consumed")
    if auth_status not in {"PUBLISHED", "CONSUMED"}:
        raise HTTPException(
            status_code=409,
            detail=f"Authorization status is {auth_status!r}, expected 'PUBLISHED'",
        )

    # --- Validate incident_id matches ---
    if auth_row["incident_id"] != body.incident_id:
        raise HTTPException(
            status_code=400,
            detail=f"Incident mismatch — authorization is for "
            f"{auth_row['incident_id']!r}, not {body.incident_id!r}",
        )
    ...
    # --- Atomically: PUBLISHED → CONSUMED (skipped on replay) ---
    if not is_replay:
        db.execute("BEGIN IMMEDIATE")
        try:
            # Re-check status under lock to prevent TOCTOU race
            locked_row = db.execute(
                "SELECT status, consumed_by FROM authorizations WHERE authorization_id=?",
                (authorization_id,),
            ).fetchone()
            if locked_row["status"] != "PUBLISHED":
                db.execute("ROLLBACK")
                if (
                    locked_row["status"] == "CONSUMED"
                    and (locked_row["consumed_by"] or "") == role_or_error
                ):
                    # Lost the race to our own role's earlier consume — same
                    # replay semantics as the pre-lock check above.
                    is_replay = True
                else:
                    raise HTTPException(
                        status_code=409,
    ...
    logger.info(
        f"[consume] Authorization {'replay acknowledged' if is_replay else 'consumed'}: "
        f"auth_id={authorization_id[:12]}..., "
        f"incident={body.incident_id}, consumed_by={role_or_error}"
    )
