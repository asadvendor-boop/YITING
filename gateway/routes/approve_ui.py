"""YITING Gateway — Human Approval Web Page.

GET  /approve/{incident_id} — Renders approval form (Caddy-authenticated).
POST /approve/{incident_id} — Consumes nonce via _do_consume_nonce().
...
logger = logging.getLogger("yiting.approve_ui")
...
def _load_response_plan(db, incident_id: str) -> dict | None:
...
<title>YITING Approval — {incident_id}</title>
<style>
...
_PLAN_CONTENT = """
<h1>🔐 Approval Required — {incident_id}</h1>
<div class="card risk-high">
  <strong>Runbook:</strong> {runbook}<br>
...
<form method="POST" id="decision-form">
  ...
  <div class="card" style="border-left: 4px solid #d29922;">
    ...
    <div style="margin: 0.5rem 0;">
      ...
      <textarea name="revision_instructions" id="revision_instructions"
                placeholder="e.g. Use circuit breaker instead of rollback, add rate limiting..."
    ...
    <button type="submit" name="decision" value="revise"
            ...
            onclick="var t=document.getElementById('revision_instructions').value.trim();if(!t){{alert('Please provide revision instructions');return false;}}if(t.length>1000){{alert('Instructions too long (max 1000 characters)');return false;}}">
  ...
  <div style="margin-top: 0.75rem;">
    <button type="submit" name="decision" value="false_alarm"
            ...
            onclick="return confirm('Are you sure this is a false alarm? This action is terminal — the incident will be closed.')">
...
_SUCCESS_CONTENT = """
<h1>✅ Approved — {incident_id}</h1>
<p class="success">{message}</p>
"""

_REJECTED_CONTENT = """
<h1>❌ Rejected — {incident_id}</h1>
<p class="error">{message}</p>
"""

_REVISION_REQUESTED_CONTENT = """
<h1>🔄 Revision Requested — {incident_id}</h1>
<p style="color:#d29922; font-weight:600;">{message}</p>
<p style="color:#8b949e;">Commander will propose a revised plan. Refresh this page when notified.</p>
"""

_FALSE_ALARM_CONTENT = """
<h1>🚫 Closed — False Alarm — {incident_id}</h1>
<p class="error">{message}</p>
<p style="color:#8b949e;">This incident has been sealed as a false alarm. No further automation will run.</p>
"""

_ERROR_CONTENT = """
<h1>⚠️ Error — {incident_id}</h1>
<p class="error">{message}</p>
"""

_NO_PENDING_CONTENT = """
<h1>ℹ️ No Pending Approval — {incident_id}</h1>
<p>No active approval challenge found for this incident.</p>
...
@router.get("/approve/{incident_id}", response_class=HTMLResponse)
async def approval_page(incident_id: str, request: Request):
    ...
    if not inc:
        return HTMLResponse(
            _APPROVAL_PAGE.format(
                incident_id=html_mod.escape(incident_id),
    ...
    auth_published = db.execute(
        ...
        "WHERE incident_id=? AND authorization_type='human_approval' "
    ...
    if auth_pending:
        ...
        esc_id = html_mod.escape(incident_id)
        return HTMLResponse(
            _APPROVAL_PAGE.format(
                incident_id=esc_id,
                content=(
                    '<h1>⏳ Pending Resume — {incident_id}</h1>'
                    '<div class="card">'
    ...
    # Load plan
    plan_info = _load_response_plan(db, incident_id)
    if not plan_info:
        return HTMLResponse(
            _APPROVAL_PAGE.format(
                incident_id=html_mod.escape(incident_id),
...
@router.post("/approve/{incident_id}", response_class=HTMLResponse)
async def approve_incident(incident_id: str, request: Request):
    ...
    Three decisions:
        ...
        revise      → seal StructuredApproval(REJECTED, reason=...) → Commander revises
    ...
    if decision == "approve":
        return await _handle_approve(
            incident_id, nonce, plan_info, approver_id, is_resume, db,
        )
    elif decision == "revise":
        return await _handle_revise(
            incident_id, nonce, plan_info, approver_id,
            revision_instructions, db,
        )
    elif decision == "false_alarm":
        return await _handle_false_alarm(
            incident_id, nonce, plan_info, approver_id, db,
        )
    else:
        return HTMLResponse(
            _APPROVAL_PAGE.format(
...
async def _handle_approve(
    incident_id: str, nonce: str, plan_info: dict,
...
async def _handle_revise(
    ...
    """Handle REJECT & REVISE — seal StructuredApproval(REJECTED), publish to room @Commander.
    ...
    try:
        ...
        if not inc or inc["state"] != "PLANNED":
            ...
            return HTMLResponse(
                _APPROVAL_PAGE.format(
                    ...
                    content=_ERROR_CONTENT.format(
                        incident_id=html_mod.escape(incident_id),
                        message=f"Cannot revise: incident state is '{inc['state'] if inc else 'NOT_FOUND'}', expected 'PLANNED'",
                    ),
                ),
                status_code=409,
            )
        ...
        if not valid:
            db.execute("ROLLBACK")
            logger.warning(
                "[approve_ui] Revise nonce rejected: %s (incident=%s)",
                reason,
                incident_id,
            )
            return HTMLResponse(
        ...
        idem_key = derive_idempotency_key(
            "gateway_rejection", incident_id, nonce,
        )

        sealed = seal_card_in_transaction(
            rejection_card, incident_id, db,
        ...
        db.execute(
            "UPDATE nonces SET invalidated=1 WHERE incident_id=? AND nonce=? AND consumed=0",
            (incident_id, nonce),
        )
        ...
        if new_state:
            ...
            db.execute(
                ...
                (new_state, now_str, incident_id),
            )
            logger.info(
                f"[approve_ui] State atomically advanced: {incident_id} → {new_state}"
            )
    ...
    # Post-commit: publish sealed card to the incident room (best-effort notification).
    # The decision is already applied (card sealed, state advanced, nonce burned).
    # Room notification lets Commander act on it; if it fails, the decision
    ...
    for attempt, delay in enumerate(ROOM_RETRY_DELAYS):
        try:
            published = await _publish_rejection_to_room(
                db, incident_id, sealed_card_hash, room_id, mention_commander=True,
            )
            if published:
                break
        except Exception as exc:
    ...
    if not published:
        ...
        # But Commander wasn't notified. Show honest 'notification pending' message.
        logger.warning(
            f"[approve_ui] Revise card sealed, state advanced to REJECTED, "
            f"but room notification failed for {incident_id}"
        )
        return HTMLResponse(
            _APPROVAL_PAGE.format(
                incident_id=html_mod.escape(incident_id),
                content=_REVISION_REQUESTED_CONTENT.format(
                    ...
                    message=(
                        ...
                        "Room notification pending — if Commander does not act, "
    ...
    logger.info(
        f"[approve_ui] Plan REJECTED (revise) for {incident_id}: "
...
async def _handle_false_alarm(
    ...
    try:
        ...
        if not inc or inc["state"] != "PLANNED":
            ...
            return HTMLResponse(
                _APPROVAL_PAGE.format(
                    ...
                    content=_ERROR_CONTENT.format(
                        incident_id=html_mod.escape(incident_id),
                        message=f"Cannot close: incident state is '{inc['state'] if inc else 'NOT_FOUND'}', expected 'PLANNED'",
                    ),
                ),
                status_code=409,
            )
        ...
        if not valid:
            db.execute("ROLLBACK")
            logger.warning(
                "[approve_ui] False alarm nonce rejected: %s (incident=%s)",
                reason,
                incident_id,
            )
            return HTMLResponse(
        ...
        idem_key = derive_idempotency_key(
            "gateway_false_alarm", incident_id, nonce,
        )

        sealed = seal_card_in_transaction(
            false_alarm_card, incident_id, db,
            idempotency_key=idem_key,
            prepared_by_role="gateway",
        )
        sealed_card_hash = sealed.card_hash
        ...
        # Create suppression rule from AlertCard fingerprint (bounded learning)
        try:
            alert_row = db.execute(
                """SELECT card_json FROM cards
                   WHERE incident_id=? AND json_extract(card_json, '$.card_type') = 'AlertCard'
                   ORDER BY sequence_number ASC LIMIT 1""",
                (incident_id,),
            ).fetchone()
            if alert_row:
                import json as json_mod
                alert_data = json_mod.loads(alert_row["card_json"])
                fp = alert_data.get("fingerprint", "")
                if fp:
                    ...
                    if not existing:
                        db.execute(
                            ...
                            (fp, "Human FALSE_ALARM determination", incident_id),
                        )
                        logger.info(
                            f"[approve_ui] Created suppression rule for fp={fp[:16]}... "
                            f"from FALSE_ALARM on {incident_id}"
                        )
        except Exception as exc:
            logger.warning(
    ...
    # Post-commit: publish to incident room (best-effort notification)
    published = False
    try:
        published = await _publish_rejection_to_room(
            db, incident_id, sealed_card_hash, room_id, mention_commander=True,
        ...
        return HTMLResponse(
            _APPROVAL_PAGE.format(
                ...
                content=_FALSE_ALARM_CONTENT.format(
                    incident_id=html_mod.escape(incident_id),
                    message=(
                        "Incident closed as false alarm. Sealed in evidence chain. "
    ...
    logger.info(f"[approve_ui] Incident {incident_id} CLOSED as FALSE ALARM")
    ...
    return HTMLResponse(
        _APPROVAL_PAGE.format(
            ...
            content=_FALSE_ALARM_CONTENT.format(
                incident_id=html_mod.escape(incident_id),
                message="Incident closed as false alarm. Sealed in the evidence chain.",
            ),
        ),
    )
...
async def _publish_rejection_to_room(
    db, incident_id: str, sealed_card_hash: str, room_id: str,
    *, mention_commander: bool = True,
) -> bool:
    """Publish a sealed StructuredApproval(REJECTED|FALSE_ALARM) to the incident room.
    ...
    if not room_id:
        logger.warning(f"[approve_ui] No incident room for {incident_id} — skipping publication")
        return False
    ...
    logger.info(
        f"[approve_ui] Rejection card published to incident room: "
        f"incident={incident_id}, message={message_id}"
    )
    ...
    try:
        ...
        updated = db.execute(
            ...
            "WHERE card_hash=? AND incident_id=? AND published_at IS NULL",
            (now_str, message_id, sealed_card_hash, incident_id),
        ).rowcount
        db.commit()
        if updated:
            logger.info(
