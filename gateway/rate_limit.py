"""Small in-process request rate limiter for the single-VM judging profile."""
    ...
    def __init__(
        ...
        *,
        ...
        self.requests_per_window = (
            ...
            else _positive_int_env("YITING_RATE_LIMIT_PER_MINUTE", 600)
        )
        self.window_seconds = (
            window_seconds
            if window_seconds is not None
            else _positive_int_env("YITING_RATE_LIMIT_WINDOW_SECONDS", 60)
        )
        if self.requests_per_window <= 0 or self.window_seconds <= 0:
            raise ValueError("YITING rate limits must be positive")
        self.trust_proxy_headers = (
            trust_proxy_headers
            if trust_proxy_headers is not None
            else os.getenv("YITING_TRUST_PROXY_HEADERS", "").lower() in {"1", "true", "yes"}
        )
