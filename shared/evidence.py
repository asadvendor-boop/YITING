"""YITING Evidence Strength Computation — Deterministic formula.
...
LLM-judged (Diagnosis assigns them against a rubric). We claim the
aggregation is deterministic, not the input judgments.
"""
from __future__ import annotations


SOURCES = ("sentry", "metrics", "deploys", "uptime")
...
def compute_evidence_strength(signals: dict, hypothesis: str | None = None) -> float:
    ...
    Args:
        signals: Dict with keys per source. Each source has:
            ...
            - relevance_score: float (0.0-1.0, LLM-judged)
    ...
    Example outputs:
        Bad deploy (all 4 anomalous, all highly relevant): ~0.90
        Suspicious deploy (3/4, mixed relevance): ~0.53
        Stale noisy alert (2/4, low relevance, old): ~0.10
    """
...
def compute_temporal_correlation(signals: dict) -> float:
    """Derive from deploy↔first-error timestamp gap. Deterministic.

    - ≤5min  → 0.95 (strong temporal link)
    - ≤15min → 0.80
    - ≤60min → 0.50
    - >60min → 0.20 (weak temporal link)
    - No deploy → 1.0 (neutral)
    """
    gap_minutes = signals.get("deploy_to_error_gap_minutes")
...
def compute_recency(signals: dict) -> float:
    """Derive from AlertCard timestamp vs current time. Deterministic.
