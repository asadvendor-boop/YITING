...
Used by all 5 agent preprocessors to filter stale messages after incident room
...
logger = logging.getLogger("yiting.replay_guard")
...
async def should_skip_stale_card(
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning(
                f"[{role}] Gateway {resp.status_code} for {incident_id} "
        ...
        if max_published_seq > this_card_seq:
            logger.info(
                ...
                f"{incident_id}: higher seq {max_published_seq} published"
            )
            return True
        return False
