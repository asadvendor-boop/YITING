"""YITING Integrity Chain — Seal-Before-Send.

Cards are sealed by the Gateway before posting to incident rooms.
The card in incident room already contains its hash — verifiable by any observer.

Flow:
    Agent → typed tool → SubmissionClient → POST /prepare/{card_type}
    → Gateway validates, enriches, seals → returns sealed card + destination
    → SubmissionClient publishes to incident room → POST /confirm with room_message_id
    → state advances.
"""
...
def seal_card(
    ...
    UNIQUE(incident_id, sequence_number) and UNIQUE(incident_id, idempotency_key)
    ...
    try:
        ...
        prev = db.execute(
            ...
            "WHERE incident_id=? ORDER BY sequence_number DESC LIMIT 1",
            (incident_id,),
        ).fetchone()
        ...
        db.execute(
            "INSERT INTO cards (incident_id, sequence_number, card_type, "
        ...
        raise IdempotencyConflict(
            ...
            f"different card type on incident {incident_id!r}"
        )
    except Exception:
        db.execute("ROLLBACK")
        raise
...
def verify_chain(incident_id: str, db: sqlite3.Connection) -> tuple[bool, list[str]]:
    """Verify the integrity of an incident's card chain.
    ...
    rows = db.execute(
        ...
        "WHERE incident_id=? ORDER BY sequence_number ASC",
        (incident_id,),
    ).fetchall()
...
def get_chain_root_hash(incident_id: str, db: sqlite3.Connection) -> str | None:
...
def get_chain_length(incident_id: str, db: sqlite3.Connection) -> int:
    """Get total number of cards in the chain."""
    row = db.execute(
        "SELECT COUNT(*) as cnt FROM cards WHERE incident_id=?",
        (incident_id,),
    ).fetchone()
