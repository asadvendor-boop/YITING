"""Local incident-room runtime for YITING agents.

Agents poll Gateway-owned incident rooms, feed messages into deterministic
...
logger = logging.getLogger("yiting.local_room_runtime")
...
class MessageEvent:
    """incident room-compatible event shape backed by a Gateway room message row."""
...
class LocalRoomAgent:
    ...
    def __init__(
        ...
        *,
        ...
        self.model = model
        self.client = IncidentRoomClient(
    ...
    async def run(self) -> None:
        logger.info(
            "[%s] Local incident-room runtime started (agent_id=%s)",
            self.role,
    ...
    async def _poll_once(self) -> None:
        rooms = await self.client.list_rooms(
            ...
            limit=int(os.getenv("YITING_ROOM_POLL_LIMIT", "100")),
        )
        for room in rooms:
