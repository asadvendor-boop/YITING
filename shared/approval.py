"""YITING Approval System — Nonce validation and authorization verification.

Both Commander and Operator independently verify approvals from the
...
# Actions that always require human approval regardless of risk level
...
def generate_nonce(length: int = 6) -> str:
    ...
    COUPLING: The Operator preprocessor regex hardcodes {6} to match this
...
def normalize_plan_for_hash(plan_data: dict) -> dict:
    ...
    Commander computes plan_hash BEFORE sealing, so all Gateway routes that
...
def requires_human_approval(
    ...
    for envelope in envelopes:
        if envelope.get("action_id") in MUTATING_ACTIONS:
            return True
    return False
...
def create_nonce(
    ...
    Atomically invalidates any previous nonce for this incident
    (plan revision = new nonce).

    Args:
        incident_id: The incident being approved.
    ...
    try:
        # Invalidate previous nonces for this incident
        db.execute(
        ...
        db.execute(
            "INSERT INTO nonces "
            "(incident_id, nonce, plan_hash, action_hash, plan_revision, expiry, consumed, invalidated) "
            "VALUES (?, ?, ?, ?, ?, ?, 0, 0)",
            (incident_id, nonce, plan_hash, action_hash, plan_revision, expiry.isoformat()),
        )
        db.execute("COMMIT")
    except Exception:
        db.execute("ROLLBACK")
...
def validate_and_consume_nonce(
    ...
    Validates (all in one BEGIN IMMEDIATE transaction):
        1. Nonce exists for this incident
    ...
    try:
        ...
        db.execute(
            ...
            (consumed_by, now.isoformat(), incident_id, nonce),
        )
        db.execute("COMMIT")
...
def validate_nonce_only(
    ...
    *,
    ...
    if require_challenge_visibility and not (
        ...
        return False, "Approval challenge is not yet visible in incident room", None
...
def create_authorization(
    ...
    db.execute(
        ...
        "(authorization_id, incident_id, plan_hash, action_hash, "
...
def validate_and_consume_authorization(
    ...
    Validates: existence, incident_id, plan_hash, action_hash,
    not expired, not consumed. Invalid request does not consume.
    """
    db.execute("BEGIN IMMEDIATE")
    try:
        row = db.execute(
            "SELECT authorization_id, incident_id, plan_hash, action_hash, "
        ...
        if row["incident_id"] != incident_id:
            db.execute("ROLLBACK")
            return False, f"Incident mismatch — auth for {row['incident_id']}, not {incident_id}"
        ...
        # Consume atomically — sync status + compatibility fields
...
def advance_authorization_lifecycle(
    ...
    authorization_id: str,
    room_message_id: str,
    target_incident_state: str,
) -> bool:
    """Two-mode state transition for incident-room publication success.
    ...
    try:
        # Read current state
        card = db.execute("SELECT published_at FROM cards WHERE card_hash=? AND incident_id=?", (card_hash, incident_id)).fetchone()
        ...
        is_target = (card["published_at"] is not None and auth["status"] == "PUBLISHED" and inc["state"] == target_incident_state)

        if is_source:
            # Execute 3 conditional updates, require rowcount=1 for each
            c1 = db.execute(
                "UPDATE cards SET published_at=?, room_message_id=? WHERE card_hash=? AND incident_id=? AND published_at IS NULL",
                (now, room_message_id, card_hash, incident_id)
            )
            c2 = db.execute(
                "UPDATE incidents SET state=?, updated_at=? WHERE incident_id=? AND state='PLANNED'",
                (target_incident_state, now, incident_id)
            )
            c3 = db.execute(
