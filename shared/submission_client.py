"""YITING SubmissionClient — Agent-side client for Gateway seal-before-send.

Every card flows through this three-phase saga:
  1. prepare(card)  → Gateway validates, enriches, seals → PrepareResult
  2. Agent publishes sealed card to the incident room (caller's responsibility)
...
Contract:
  ...
  - Used by ALL agent adapters: Recorder, Triage, Diagnosis,
    Safety Reviewer, Commander, Operator.
...
logger = logging.getLogger("yiting.submission_client")
...
class PrepareResult:
    ...
    sequence_number: int     # Position in the incident's card chain
    incident_id: str         # Incident this card belongs to
    agent_role: str          # Role resolved from the agent key
    room_id: str | None      # Room to publish to (None if room not yet created)

    @property
    def room_alias_id(self) -> str | None:
        """Compatibility alias for schemas that still expose room_alias_id."""
        return self.room_id
...
class ConfirmResult:
    ...
    card_hash: str
    message_id: str
    new_state: str | None    # New incident state (None if no transition)
...
class SubmissionClient:
    ...
    Usage:
        ...
        # 1. Prepare (seal)
        result = await client.prepare(alert_card)
    ...
    async def prepare(
        ...
        *,
        ...
        Args:
            card: Any CardBase subclass (AlertCard, Assessment, etc.)
        ...
        return PrepareResult(
            ...
            room_id=(
                ...
                or data.get("destination", {}).get("room_alias_id")
            ),
        )
    ...
    async def confirm(
        ...
        *,
        ...
        room_alias_id: str | None = None,
    ) -> ConfirmResult:
        """Confirm that a sealed card was published to the incident room.

        This advances the incident state machine. Call AFTER the card
        is visible in the incident room.

        Args:
            submission_id: The submission_id from PrepareResult.
            incident_id: The incident this card belongs to.
            card_hash: The card_hash from PrepareResult.
            message_id: The incident-room message id.
        ...
        resolved_room_id = room_id or room_alias_id
...
def format_card_message(sealed_card: dict) -> str:
    """Format a sealed card as an incident-room message.

    This is the standard message format for YITING cards in incident rooms:
    a short human-readable summary followed by the full sealed card JSON.
    """
    card_type = sealed_card.get("card_type", "Card")
    incident_id = sealed_card.get("incident_id", sealed_card.get("alert_id", "unknown"))
    card_hash = sealed_card.get("card_hash", "")[:12]
    seq = sealed_card.get("sequence_number", "?")

    # Build human-readable summary based on card type
    summary_parts = [f"**{card_type}** for incident `{incident_id}` (seq {seq}, hash `{card_hash}…`)"]
