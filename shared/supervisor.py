"""YITING Agent Supervisor — process-level resilience wrapper.
...
logger = logging.getLogger("yiting.supervisor")
...
async def run_with_supervisor(create_agent_fn, role: str, meta: dict | None = None):
    """Run an agent with crash recovery and best-effort heartbeat.
    ...
    while True:
        try:
            ...
            # Don't retry auth failures — exit so yiting doctor catches them
